...
def build_deepresearch_kernel_v2_flow(
    ...
    *,
    ...
    researcher = Step(
        id="researcher",
        brief="Own the DeepResearch workspace: gather evidence, maintain research state, write or repair the report, and hand off to review.",
        inputs=[
        ...
        read=["contract", "product_contract", "manuals", "sources", "reports", "claims", "reviews", "judge", "state"],
        write=["sources", "reports", "claims", "state"],
        tools=tools,
    ...
    reviewer = Step(
        ...
        read=["contract", "product_contract", "rubrics", "sources", "reports", "claims", "state"],
        write=["reviews"],
    ...
    judge = Step(
        ...
        read=["contract", "product_contract", "rubrics", "sources", "reports", "claims", "reviews", "state"],
        write=["judge"],
    ...
    return Flow(
        id="deepresearch-v2",
        steps=[researcher, reviewer, judge],
        routes={
...
def _researcher_brief(request: AcademicResearchRequest) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "You own the research workspace for this request. Do semantic research inside the hard MissionForge boundaries.",
            "Use live academic/repo tools when available. Use read/write/edit to maintain durable workspace artifacts.",
            "Work in phases: plan -> evidence batch -> synthesis -> repair. Do not keep one monolithic search loop alive for the whole run.",
            "When evidence questions are independent, batch them in the same PiWorker turn and use multiple tool calls in parallel instead of serializing them one by one.",
            "After each meaningful evidence batch, refresh the durable workspace artifacts so later review can inspect the current posterior.",
            "If the current batch is enough to improve the draft, switch from search to synthesis and writing.",
            ...
            *_researcher_intensity_guidance(profile.intensity),
            ...
            "These are quality targets, not permission to exhaust the runtime. Prefer a reviewable partial synthesis over no artifacts.",
            "If source coverage is weaker than the target, write a useful report anyway and make limitations explicit in `reports/source_gaps.md`.",
...
def _researcher_intensity_guidance(intensity: ResearchIntensity) -> list[str]:
    if intensity is ResearchIntensity.INTENSIVE:
        return [
            ...
            "Include a repo audit map in the final report: target repo, audited files/paths, entrypoints, agent/workflow loop, tool interfaces, citation/evidence handling, renderer/publication path, observability/logging, and confidence.",
...
def _fixture_report(request: Mapping[str, Any], source_packet: Mapping[str, Any]) -> str:
    ...
    if language.startswith("zh"):
        return (
            ...
            "## 范围与方法\n\n"
            "本报告验证 Kernel v2 的白盒编排边界，并使用夹具来源说明引用链路 [S1]。\n\n"
            "## 证据基础\n\n"
            "证据包包含一个夹具来源，用于验证 source packet、报告和审计 ref 的连接 [S1]。\n\n"
            "## 主要研究路线\n\n"
            "真实运行中该章节由 researcher PiWorker 负责综合，不由 Python 判断语义充分性 [S1]。\n\n"
            "## 对比矩阵\n\n"
            "| 路线 | 证据 |\n| --- | --- |\n| Kernel v2 fixture | [S1] |\n\n"
            "## 反证与失败模式\n\n"
            "夹具模式不声称完成真实研究，只验证控制流和 artifact 边界。\n\n"
            "## 局限与证据缺口\n\n"
