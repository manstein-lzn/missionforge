...
import { assertNoSymlinkSegments, guardWorkspacePath, refIsUnder, } from "./permissions.js";
const SANDBOX_ROOT = "/workspace";
export function createBubblewrapBashOperations(options) {
...
async function buildBubblewrapPlan(options) {
    ...
    const seccompFd = options.permissionManifest.network_policy === "disabled" ? getNetworkBlockSeccompFd() : null;
    const args = [
        "--unshare-user-try",
        "--unshare-pid",
        "--unshare-uts",
    ...
    for (const ref of writableRoots) {
        await appendWritableMount(args, options.workspaceRoot, ref, options.knownFileRefs, options.knownDirectoryRefs);
    }
...
function readonlyMountRoots(manifest, writableRoots) {
    return sortedRoots(manifest.readable_refs).filter((readRef) => !writableRoots.some((writeRef) => refIsUnder(readRef, writeRef)));
}
async function appendReadableMount(args, workspaceRoot, ref) {
...
async function appendWritableMount(args, workspaceRoot, ref, knownFileRefs, knownDirectoryRefs) {
...
async function appendDeniedMount(args, workspaceRoot, ref, knownDirectoryRefs) {
    const hostPath = safeExistingRefPath(workspaceRoot, ref, { allowMissingLeaf: true });
...
async function ensureWritableHostPath(path, asDirectory) {
    try {
...
function safeExistingRefPath(workspaceRoot, ref, options) {
    const path = resolveWorkspaceRef(workspaceRoot, ref);
    assertNoSymlinkSegments(workspaceRoot, path, options);
    return path;
}
function sandboxPathForHostPath(workspaceRoot, path) {
...
function sandboxRefPath(ref) {
    return `${SANDBOX_ROOT}/${ref}`;
}
function sanitizeSandboxEnv(env) {
    const result = {
        PATH: "/bin:/usr/bin:/usr/local/bin",
        HOME: "/tmp",
        TMPDIR: "/tmp",
...
function isDirectoryMountRef(ref, knownFileRefs, knownDirectoryRefs) {
...
function deriveDirectoryRoots(refs) {
...
function collectMountDirs(refs, knownDirectoryRefs) {
    const dirs = new Set([""]);
...
function getNetworkBlockSeccompFd() {
    if (networkBlockSeccompFd !== null)
...
let networkBlockSeccompFd = null;
let networkBlockSeccompPath = null;
function getNetworkBlockSeccompProgram() {
