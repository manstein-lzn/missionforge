...
import { createContextSnapshotTool } from "./context-snapshot.js";
...
export async function createMissionForgeTools(options) {
    const cwd = options.workspaceRoot;
    const effectiveManifest = permissionManifestFromSandboxProfile(options.permissionManifest, options.sandboxProfile);
    ...
    if (effectiveManifest.allowed_commands.length > 0 && canUseTool(effectiveManifest.allowed_tools, "bash")) {
        if (options.sandboxProfile && options.sandboxProfile.mode !== "bubblewrap") {
            throw new Error(`bash requires sandbox_profile.mode=bubblewrap; got ${options.sandboxProfile.mode}`);
        }
        ...
        toolsByName.set("bash", createBashTool(cwd, {
    ...
    for (const tool of options.extensionTools ?? []) {
        if (!canUseTool(effectiveManifest.allowed_tools, tool.name))
            continue;
...
export function permissionManifestFromSandboxProfile(permissionManifest, sandboxProfile) {
    if (!sandboxProfile)
        return permissionManifest;
    if (sandboxProfile.mode === "unsupported") {
        throw new Error("sandbox_profile.mode must be supported");
    }
    const profileAllowedTools = sandboxProfile.allowed_tools;
    const manifestAllowedTools = permissionManifest.allowed_tools;
...
function canUseTool(allowedTools, toolName) {
    ...
    if (effectiveAllowedTools.includes(toolName))
        return true;
    if (toolName === "read_text" || toolName === "read_json")
        return effectiveAllowedTools.includes("read");
    if (toolName === "write_text" || toolName === "write_json")
        return effectiveAllowedTools.includes("write");
    return false;
}
function createGatewayExtensionTool(tool, gateway) {
    return {
        ...tool,
...
function createGatewayReadOperations(gateway) {
    return {
...
function createGatewayEditOperations(gateway) {
    return {
        readFile: (absolutePath) => readFile(gateway.authorizeReadPath("edit", absolutePath)),
        writeFile: (absolutePath, content) => writeFile(gateway.authorizeWritePath("edit", absolutePath), content, "utf-8"),
        access: async (absolutePath) => {
            await readFile(gateway.authorizeReadWritePath("edit", absolutePath), { flag: "r" });
        },
    };
}
function createGatewayWriteOperations(gateway) {
    return {
        writeFile: (absolutePath, content) => writeFile(gateway.authorizeWritePath("write", absolutePath), content, "utf-8"),
        mkdir: async (dir) => {
            await mkdir(gateway.authorizeWriteContainerPath("write", dir), { recursive: true });
        },
    };
}
function createGatewayBashOperations(gateway, operations, toolTimeoutSeconds) {
    return {
...
export function guardPath(workspaceRoot, path) {
    return guardWorkspacePath(workspaceRoot, path);
}
export async function writeExpectedArtifact(workspaceRoot, ref, content, permissionManifest) {
