...
export async function runMissionForgePiAgent(input, workspaceRoot) {
    const provider = resolveProviderConfig();
    const maxTurns = effectiveMaxTurns(input, provider.maxTurns);
    const mainTurnLimit = mainTurnLimitWithCompletionReserve(maxTurns, input.call_spec.expected_outputs.length, DEFAULT_COMPLETION_RETRY_LIMIT);
    ...
    let contextCheckpointRef = null;
    let cancellationReason = null;
    let latestProjectionDiagnostics = null;
    let unregisterFaux;
    ...
    try {
        ...
        if (loadedReport.rejected_extensions.length > 0) {
            throw new Error(`extension load rejected: ${loadedReport.rejected_extensions.map((record) => `${record.grant_id}:${record.reason}`).join(", ")}`);
        }
        ...
        if (provider.mode === "faux") {
            ...
            if (outputs.length === 0)
                throw new Error("PiWorker call requires at least one expected output");
            faux.setResponses([
                fauxAssistantMessage(outputs.map((outputRef, index) => fauxToolCall("write", {
                    path: outputRef,
        ...
        const loopConfig = {
            ...
            convertToLlm: async (messages) => convertAgentMessagesToLlm(messages),
            transformContext: async (messages) => {
                const projected = projector.project(await stripUnreplayableResponsesReasoning(messages), systemPrompt, provider.model, input);
            ...
            afterToolCall: async (context) => {
                ...
                if (observation)
                    await recorder.recordToolObservation(observation);
                return undefined;
            },
            shouldStopAfterTurn: async (turnContext) => {
                projector.project(await stripUnreplayableResponsesReasoning(turnContext.context.messages), systemPrompt, provider.model, input);
                latestProjectionDiagnostics = projector.diagnostics(input);
        ...
        const recordEvent = async (event) => {
            ...
            if (event.type === "turn_end" &&
                ...
                contextCheckpointRef = await recorder.writeContextCheckpoint(`turn_count >= ${provider.compactAfterTurns}`, latestProjectionDiagnostics);
            }
            if (event.type === "turn_end" &&
        ...
        const runPrompt = async (prompt, currentMessages) => runAgentLoopWithProviderRetry({
    ...
    const cancellationShouldFail = cancelled &&
        (failures.length > 0 ||
            missingOutputs.length > 0);
    const artifactsAreCompleteAfterTransientFailure = !cancelled &&
        failures.length > 0 &&
        missingOutputs.length === 0 &&
...
function pushFailures(target, candidates) {
...
export function isTransientProviderFailure(message) {
    ...
    return (normalized.includes("openai api error (429)") ||
...
async function runAgentLoopWithProviderRetry(options) {
    let attempt = 0;
    for (;;) {
        try {
            return await runAgentLoop(options.userMessages, { systemPrompt: options.systemPrompt, messages: options.currentMessages, tools: options.tools }, options.loopConfig, options.recordEvent, undefined, streamSimple);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            if (attempt >= options.maxRetries || !isTransientProviderFailure(message))
                throw error;
            attempt += 1;
...
async function maybeWriteContextCheckpoint(options) {
    if (options.alreadyWritten())
        return null;
    if (options.diagnostics.recommended_action === "continue")
        return null;
    try {
...
function checkpointReason(diagnostics) {
    return [
...
function runtimeKnownFileRefs(input) {
    return [
        input.input_ref,
        input.output_ref,
        input.session_ref,
...
function contextEngineKnownFileRefs(input) {
...
export async function loadContextEngineProviderText(input, workspaceRoot) {
    const engine = input.context_engine;
    if (!engine?.enabled)
        return "";
    ...
    const view = requireJsonObject(await readJsonFile(enforcer.ensureReadPath(resolveWorkspaceRef(workspaceRoot, engine.context_view_ref))), "context_engine.context_view");
    const compileResult = requireJsonObject(await readJsonFile(enforcer.ensureReadPath(resolveWorkspaceRef(workspaceRoot, engine.context_compile_result_ref))), "context_engine.context_compile_result");
...
async function renderContextEngineProviderText(input, workspaceRoot, viewRef, view, compileResult, enforcer) {
    const engine = input.context_engine;
    const lines = [
    ...
    for (const [bucketName, fieldName] of [
        ...
        lines.push(await renderContextSegmentBucket(workspaceRoot, enforcer, bucketName, requireArrayField(view, fieldName, `context_view.${fieldName}`)));
    }
    ...
    if (engine.context_cache_layout_ref)
        lines.push(`context_cache_layout_ref: ${engine.context_cache_layout_ref}`);
    if (engine.context_pressure_ref)
        lines.push(`context_pressure_ref: ${engine.context_pressure_ref}`);
    if (engine.context_epoch_ref)
        lines.push(`context_epoch_ref: ${engine.context_epoch_ref}`);
    return lines.join("\n");
}
async function renderContextSegmentBucket(workspaceRoot, enforcer, bucketName, values) {
    ...
    for (const item of values.slice(0, 20)) {
        ...
        if (boundedProjection)
            lines.push(boundedProjection);
    }
    if (values.length > 20)
        lines.push(`- additional_segment_count=${values.length - 20}`);
    return lines.join("\n");
}
async function boundedContextProjectionText(workspaceRoot, enforcer, segment, bodyRef) {
    if (!bodyRef)
        return "";
...
function projectionLabelForSegment(kind, sourceKind) {
    if (kind === "artifact_preview" && sourceKind === "working_set")
        return "working_set_projection";
    if (kind === "tool_observation" && sourceKind === "tool_output_projection")
        return "tool_output_projection";
    return "";
}
function compiledSegmentHash(segment, ref) {
...
function contextViewContentForHash(view) {
...
function stableJsonString(value) {
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            throw new Error("stable_json value must not contain non-finite numbers");
...
function boundedProjectionLines(text, maxChars = 4000, maxLines = 80) {
...
function requireArrayField(record, key, field) {
    const value = record[key];
    if (!Array.isArray(value))
        throw new Error(`${field} must be an array`);
    return value;
}
function requireRefArrayField(record, key, field) {
    return requireArrayField(record, key, field).map((item, index) => requireRef(item, `${field}[${index}]`));
}
function requireStringField(record, key, field) {
    const value = record[key];
    if (typeof value !== "string" || value.length === 0)
        throw new Error(`${field} must be a non-empty string`);
    return value;
}
function optionalRefField(value, field) {
    if (value === undefined || value === null || value === "")
        return null;
    return requireRef(value, field);
}
function requireNonNegativeNumberField(record, key, field) {
    const value = record[key];
...
function runtimeKnownDirectoryRefs(input) {
    return [
...
export async function stripUnreplayableResponsesReasoning(messages) {
...
export function mainTurnLimitWithCompletionReserve(maxTurns, expectedOutputCount, completionRetryLimit = DEFAULT_COMPLETION_RETRY_LIMIT) {
    if (expectedOutputCount <= 0 || maxTurns <= 1 || completionRetryLimit <= 0)
        return maxTurns;
    const reserve = Math.min(completionRetryLimit, maxTurns - 1);
    return Math.max(1, maxTurns - reserve);
}
function buildSystemPrompt(input, contextEngineText = "") {
    const lines = [
...
function effectiveMaxTurns(input, fallback) {
    const value = input.piworker_call?.runtime_budget?.max_turns;
    return typeof value === "number" && Number.isInteger(value) && value > 0 ? value : fallback;
}
export async function promptForMissingExpectedOutputs(options) {
    let missingOutputs = await collectMissingExpectedOutputRefs(options.input, options.workspaceRoot, await options.changedRefs?.());
    let retryCount = 0;
    while (missingOutputs.length > 0 &&
        ...
        missingOutputs = await collectMissingExpectedOutputRefs(options.input, options.workspaceRoot, await options.changedRefs?.());
    }
    return { missingOutputs, retryCount };
}
export function buildCompletionRetryPrompt(input, missingOutputs, retryCount) {
    const lines = [
    ...
    lines.push("Write only the missing expected outputs listed above.", "Do not answer in text instead of writing artifacts. Use the available file tools to create or update the missing refs.");
    if (input.piworker_call?.role === "judge_piworker") {
        lines.push("As judge_piworker, write a complete judge report JSON object at the missing report ref using the visible judge context refs.");
    }
    return lines.join("\n");
}
export async function missingExpectedOutputRefs(input, workspaceRoot) {
    return collectMissingExpectedOutputRefs(input, workspaceRoot, undefined);
}
async function collectMissingExpectedOutputRefs(input, workspaceRoot, changedRefsSnapshot) {
...
async function fallbackFauxArtifact(input, workspaceRoot, failures) {
    const firstOutput = input.call_spec.expected_outputs[0];
    if (!firstOutput)
        return;
    try {
        await writeExpectedArtifact(workspaceRoot, firstOutput, `pi-agent-runtime fallback faux artifact for ${input.call_id}\n`, input.permission_manifest);
    }
    catch (error) {
        failures.push(error instanceof Error ? error.message : String(error));
    }
