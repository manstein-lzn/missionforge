...
export function buildContextBudgetDiagnostics(input) {
    const contextWindow = positiveInteger(input.modelContextWindow, 128000);
    const reservedOutputTokens = clampReserve(positiveInteger(input.modelMaxOutputTokens, DEFAULT_RESERVED_OUTPUT_TOKENS), contextWindow);
...
function allocateBudget(usableInputBudget) {
...
function clampReserve(value, contextWindow) {
    if (contextWindow <= 1)
        return 0;
