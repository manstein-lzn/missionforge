...
export async function buildRuntimeOutput(options) {
    ...
    for (const ref of changedRefs) {
        if (produced.has(ref))
            continue;
        if (!options.input.call_spec.allowed_scope.includes(ref))
            continue;
    ...
    const failures = options.statusOverride === "cancelled"
        ? [...options.failures]
        : [...options.failures, ...missingOutputs.map((ref) => `expected output was not produced: ${ref}`)];
    const status = options.statusOverride ?? (failures.length === 0 ? "completed" : "failed");
    const verificationStatus = status === "completed" && failures.length === 0 ? "not_run" : "failed";
    const output = {
        ...
        recommended_next_steps: options.recommendedNextSteps ?? (status === "completed" ? [] : ["Inspect pi-agent-runtime artifacts before retrying."]),
...
export async function writeRuntimeOutput(workspaceRoot, input, output) {
