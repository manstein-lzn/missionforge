...
import {
  ...
  type LongMemoryCatalogHit,
  type LongMemoryPacket,
...
export interface LongMemoryContext {
  packet: LongMemoryPacket | null;
  message: AgentMessage | null;
  diagnostics: LongMemoryDiagnostics;
}

export interface LongMemoryDiagnostics {
...
export async function loadLongMemoryContext(input: RuntimeInput, workspaceRoot: string): Promise<LongMemoryContext> {
  ...
  if (estimatedTokens > packet.budget_tokens) {
    throw new Error(
      `long_memory_packet_ref exceeds budget_tokens: estimated ${estimatedTokens}, budget ${packet.budget_tokens}`,
    );
  }
  return {
    packet,
...
export function parseLongMemoryPacket(value: unknown, input: RuntimeInput, packetRef: string): LongMemoryPacket {
  ...
  const budgetTokens = requirePositiveInteger(
    data.budget_tokens ?? DEFAULT_LONG_MEMORY_PACKET_BUDGET_TOKENS,
    "long_memory_packet.budget_tokens",
  );
  const scope = parseScope(data.scope, input);
  const memories = requireArray(data.memories ?? [], "long_memory_packet.memories").map((item, index) =>
    parseMemoryRecord(item, `long_memory_packet.memories[${index}]`),
  );
  const catalogHits =
    data.catalog_hits === undefined || data.catalog_hits === null
      ? undefined
      : requireArray(data.catalog_hits, "long_memory_packet.catalog_hits").map((item, index) =>
          parseCatalogHit(item, `long_memory_packet.catalog_hits[${index}]`),
        );
  const packetDeclaredRef =
    data.packet_ref === undefined || data.packet_ref === null
      ? undefined
...
export function renderLongMemoryPacket(packet: LongMemoryPacket, packetRef: string): string {
  const lines = [
  ...
  if (packet.memories.length > 0) {
    ...
    for (const memory of packet.memories) {
      lines.push(
        ...
        `  why_relevant: ${memory.why_relevant}`,
        `  source_refs: ${memory.source_refs.join(", ")}`,
      );
      if (memory.created_at) lines.push(`  created_at: ${memory.created_at}`);
  ...
  if (packet.catalog_hits && packet.catalog_hits.length > 0) {
    ...
    for (const hit of packet.catalog_hits) {
      ...
      if (hit.turn_range) lines.push(`  turn_range: ${hit.turn_range[0]}..${hit.turn_range[1]}`);
      if (hit.topics && hit.topics.length > 0) lines.push(`  topics: ${hit.topics.join(", ")}`);
      if (hit.artifact_refs && hit.artifact_refs.length > 0) {
        lines.push(`  artifact_refs: ${hit.artifact_refs.join(", ")}`);
      }
      if (hit.hash) lines.push(`  hash: ${hit.hash}`);
    }
  }
  return `${lines.join("\n")}\n`;
}

export function degradedLongMemoryDiagnostics(reason: string): LongMemoryDiagnostics {
  return {
...
function parseScope(value: unknown, input: RuntimeInput): LongMemoryPacket["scope"] {
...
function parseMemoryRecord(value: unknown, field: string): LongMemoryRecord {
  ...
  return {
    ...
    confidence: confidence as LongMemoryRecord["confidence"],
    status: status as LongMemoryRecord["status"],
...
function parseCatalogHit(value: unknown, field: string): LongMemoryCatalogHit {
  const data = requireObject(value, field);
  const turnRange =
    data.turn_range === undefined || data.turn_range === null
      ? undefined
...
function parseTurnRange(value: unknown, field: string): [number, number] {
  const items = requireArray(value, field);
  if (items.length !== 2) throw new Error(`${field} must have two entries`);
  const start = requireNonNegativeInteger(items[0], `${field}[0]`);
  const end = requireNonNegativeInteger(items[1], `${field}[1]`);
  if (end < start) throw new Error(`${field}[1] must be greater than or equal to ${field}[0]`);
  return [start, end];
}

function rejectAuthorityOverride(text: string, field: string): string {
