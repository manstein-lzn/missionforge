...
export type ToolGatewayDecisionStatus = "allowed" | "denied";

export interface ToolGatewayDecision {
  schema_version: typeof TOOL_GATEWAY_DECISION_SCHEMA_VERSION;
  decision_id: string;
  created_at: string;
  manifest_id: string;
  tool_name: string;
  operation: ToolGatewayOperation;
  status: ToolGatewayDecisionStatus;
  ref?: string;
  command_hash?: string;
  cwd_ref?: string;
  env_names?: string[];
...
export interface ToolGatewayOptions {
  ...
  onDecision?: (decision: ToolGatewayDecision) => void;
}

export class ToolGateway {
  private readonly enforcer: ToolPermissionEnforcer;
  private readonly decisions: ToolGatewayDecision[] = [];
  private sequence = 0;
  ...
  getDecisions(): ToolGatewayDecision[] {
  ...
  private record(
    decision: Omit<ToolGatewayDecision, "schema_version" | "decision_id" | "created_at" | "manifest_id">,
  ): void {
    const item: ToolGatewayDecision = {
  ...
  private refForPath(path: string): string | undefined {
    try {
      const safePath = guardWorkspacePath(this.options.workspaceRoot, path);
...
function safeReason(error: unknown): string {
  ...
  if (message.includes("outside writable roots")) return "ref_outside_writable_roots";
  if (message.includes("Path escapes MissionForge workspace")) return "path_escapes_workspace";
  if (message.includes("symlink")) return "path_crosses_symlink";
